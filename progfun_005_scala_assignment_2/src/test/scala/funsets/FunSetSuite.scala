...
class FunSetSuite extends FunSuite {
  ...
  test("forall test") {
    new TestSets {
      var s = (x: Int) => (x >= -200 && x <= 300)
      var p1 = (x: Int) => (x >= -100 && x <= 400)
      var p2 = (x: Int) => (x >= -300 && x <= 300)
  ...
  test("exists test") {
    new TestSets {
      var s = (x: Int) => (x >= -200 && x <= 300)
      var p1 = (x: Int) => (x >= -100 && x <= 400)
      var p2 = (x: Int) => (x < -200 && x > 300)
      assert(exists(s, p1), "Exist 1")
      assert(!exists(s, p2), "Exist 2")
    }
  }
  
  test("map test") {
    new TestSets {
      var s = (x: Int) => (x >= -20 && x <= 10)
      var f = (x: Int) => x + 50
      var r = map(s, f)
      //printSet(r)
      def iter(i: Int) {
        if (i > 60) {}
        else assert(r(i), "map " + i)
      }      
      iter(30)
    }
  }
